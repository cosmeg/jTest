...
-- ldb display for aimed shot cast time
-- TODO move to a separate addon which only loads for hunters
local AIMED_SHOT_SPELL_ID = 19434
local AIMED_SHOT_ICON = "Interface\\Icons\\INV_Spear_07"
local STEADY_SHOT_ICON = "Interface\\Icons\\Ability_Hunter_SteadyShot"

local ldb = LibStub:GetLibrary("LibDataBroker-1.1")
local dataobj = ldb:NewDataObject("AI", {
	type = "data source",
	text = "0",
...
function f:Update()
	local _, _, _, _, _, _, castTime, _, _ = GetSpellInfo(AIMED_SHOT_SPELL_ID)
	local secs = castTime / 1000
	dataobj.value = secs
	dataobj.text = string.format("%.1f", secs)
	if secs < 1.9 then
		dataobj.icon = AIMED_SHOT_ICON
	else
		dataobj.icon = STEADY_SHOT_ICON
	end
end
